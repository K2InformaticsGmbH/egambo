...
-export([ gen/3     % generate non-periodic win modules
        ]).
...
gen(W, H, R) ->
    file:write_file("src/" ++ atom_to_list(egambo_tictac:win_module(W, H, R, false)) ++ ".erl", 
      io_lib:format("-module(~s).~n~n-export([win/2]).~n~n% generated in ~s~n~n~s~s~n",
        [ egambo_tictac:win_module(W, H, R, false)
        , ?MODULE
        , list_to_binary(
            [ gen_render(W,H,R,L) 
              || 
