...
-type egGameResult()            :: #{id=>egGameId(), status=>egGameStatus(), board=>binary(), players=>[integer()], aliases=>[integer()], scores=>[egPlayerScore()]}.
...
-record(egGameCategory, { cid      = <<>>  :: binary()   % game category id
...
-record(egGameType, { tid      = <<>>  :: binary()            % game type id
                    , tname    = <<>>  :: binary()            % game type name
                    , cid      = <<>>  :: binary()            % game category id
...
-record(egGame, { gid      = 0              :: integer()           % unique game id using ramdom integer
                , tid      = <<>>           :: binary()            % game type id
                , cid      = <<>>           :: binary()            % game category id
                ...
                , ialiases = [$X, $O]       :: [integer()]         % initial integer aliases (codes) of players matching initial movers order
